...
- **Nivel principal**: pruebas de integración sobre la app Express real (levantada en
  ...
  cliente.
- **Unitarias**: retrieval, validación de config y validación de catálogo.
- **Sin red ni secretos**: la suite corre sin `OPENAI_API_KEY` (ejercita el modo
  catálogo) y con un proveedor falso inyectado para el modo IA. `npm test` debe pasar
...
| RF-02 Retrieval | `test/retrieval.test.js` — relevancia, insensibilidad a tildes/mayúsculas, top-K, umbral (sin resultados) |
...
| RF-04 Catálogo | `test/tramites.test.js` — listado, filtro `?q=`, ficha por id, 404, arranque falla con catálogo inválido (`test/catalog.test.js`) |
...
| RNF-02 Seguridad | `test/security.test.js` — headers de seguridad presentes, 413 por payload grande, formato de error uniforme, 429 al exceder rate limit |
